...
def compare_logit_coefficients(model1, model2, var_name):
   
    """
    Compare the marginal effects (dy/dx) of a specified variable from two logit models
    to determine if their confidence intervals overlap at different significance levels.

    Parameters:
    -----------
    model1 : statsmodels.discrete.discrete_model.BinaryResultsWrapper
        The first logit model results object.
    model2 : statsmodels.discrete.discrete_model.BinaryResultsWrapper
        The second logit model results object.
    var_name : str
        The name of the variable to compare between the two models.

    Returns:
    --------
    str
        A string indicating the significance level at which the marginal effects
        are statistically different based on confidence interval overlap.

    Notes:
    ------
    - The function uses the `get_margeff()` method to extract marginal effects and
      standard errors from the models.
    - If the variable is not present in either model, a message indicating the
      absence of the variable is returned.
    - Confidence intervals are calculated for 99%, 95%, and 90% levels, and the
      lowest significance level of difference is returned if applicable.
    """
    
    # Check if the variable exists in both models
    if var_name not in model1.params or var_name not in model2.params:
        return f"Variable '{var_name}' not found in one or both models."

    # Extract coefficients and standard errors
    beta1, se1 = model1.get_margeff().summary_frame()['dy/dx'][var_name], model1.get_margeff().summary_frame()['Std. Err.'][var_name]
    beta2 = model2.get_margeff().summary_frame()['dy/dx'][var_name]

    # Define critical values for different significance levels
    critical_values = {
        0.01: norm.ppf(1 - 0.01 / 2),
        0.05: norm.ppf(1 - 0.05 / 2),
        0.10: norm.ppf(1 - 0.10 / 2),
    }

    # Check confidence intervals for each significance level
    for alpha, z in critical_values.items():
        ci1_lower, ci1_upper = beta1 - z * se1, beta1 + z * se1
        #ci2_lower, ci2_upper = beta2 - z * se2, beta2 + z * se2

        # If confidence intervals do not overlap, return the corresponding significance level
        if not (ci1_upper < beta2 < ci1_upper):
            if alpha == 0.01:
                return "Different at 1%"
            elif alpha == 0.05:
                return "Different at 5%"
            elif alpha == 0.10:
                return "Different at 10%"

    # If all confidence intervals overlap, return "Not different"
    return "Not different"


def compare_ols_coefficients(model1, model2, var_name):
    """
    Compare the coefficients of a specified variable from two OLS models to determine
    if their confidence intervals overlap at different significance levels.

    Parameters:
    -----------
    model1 : statsmodels.regression.linear_model.RegressionResultsWrapper
        The first OLS regression results object.
    model2 : statsmodels.regression.linear_model.RegressionResultsWrapper
        The second OLS regression results object.
    var_name : str
        The name of the variable to compare between the two models.
    ...
    --------
    str
        A string indicating the significance level at which the coefficients
        are statistically different based on confidence interval overlap.

    Notes:
    ------
    - Confidence intervals are calculated using heteroscedasticity-robust standard errors (`HC1_se`).
    - If the variable is not present in either model, a message indicating the
      absence of the variable is returned.
    - The function checks confidence intervals at 99%, 95%, and 90% levels, and
      returns the lowest significance level of difference if applicable.
    """
    ...
    # Extract coefficients and standard errors
    beta1, se1 = model1.params[var_name], model1.HC1_se[var_name]
    beta2 = model2.params[var_name]
    ...
    for alpha, z in critical_values.items():
        ...
        # If confidence intervals do not overlap, return the corresponding significance level
        if not (ci1_lower < beta2 < ci1_upper):
            if alpha == 0.01:
                return "Different at 1%"
            elif alpha == 0.05:
                return "Different at 5%"
